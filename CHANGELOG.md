...
- **Versioning Infrastructure**
  ...
  - Deprecation warning system via HTTP headers for deprecated endpoints

- **Core Python API (port 8100)**
  - `/v1/chat/completions` - OpenAI-compatible endpoint (frozen contract)
  - `/v1/agents/send` - Primary agent dispatch endpoint
  - `/v1/agents/orchestrate` - Multi-agent orchestration
  - `/v1/agents/providers` - List active LLM providers
  - `/v1/agents/metrics` - Global metrics and statistics
  - `/v1/cache/stats` - Cache performance metrics
...
### Breaking Changes
- **[BREAKING]** None - this is the initial versioned release
