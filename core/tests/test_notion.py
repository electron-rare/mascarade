...
async def test_search(notion_client):
    notion_client._client.search = AsyncMock(return_value={
        "results": [
            {
                "object": "page",
                "id": "abc-123",
...
async def test_read_page(notion_client):
    notion_client._client.blocks.children.list = AsyncMock(return_value={
        "results": [
            {"type": "paragraph", "paragraph": {"rich_text": [{"plain_text": "Content here"}]}},
        ]
    })
