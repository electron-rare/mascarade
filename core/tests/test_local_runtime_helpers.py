...
def test_prepare_runtime_step_prints_switch_commands(tmp_path):
    home = tmp_path / "home"
    model_root = (
        home / "Models" / "mascarade" / "apple-llm" / "Qwen3.5-4B-ONNX-q4f16" / "onnx"
    )
    ...
    tokenizer_root = (
        home / "Models" / "mascarade" / "apple-llm" / "Qwen3.5-4B-ONNX-q4f16"
    )
    ...
    assert (
        "python3 /tmp/ane/scripts/run_next_lots.py --resume /tmp/state.json"
        in result.stdout
    )
...
def test_ensure_apple_models_check_only_is_idempotent(tmp_path):
    ...
    qwen05_root = (
        home / "Models" / "mascarade" / "apple-llm" / "Qwen2.5-0.5B-Instruct-model"
    )
    ...
    mistral = (
        home / "Models" / "mascarade" / "apple-llm" / "StatefulMistral7BInstructInt4"
    )
    mistral.mkdir(parents=True)
    (mistral / "StatefulMistral7BInstructInt4.mlpackage").write_text(
        "x", encoding="utf-8"
    )
