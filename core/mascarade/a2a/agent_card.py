...
class AgentCard:
    ...
    authentication: dict[str, Any] = field(default_factory=lambda: {"type": "bearer", "required": False})
...
def generate_agent_card(agent: Any, base_url: str = "http://localhost:8100") -> AgentCard:
    ...
    return AgentCard(
        name=name, description=desc, url=f"{base_url}/a2a",
        capabilities=["chat", "task", f"category:{category}"],
        skills=skills, provider=f"mascarade/{model}" if model else "mascarade",
    )
