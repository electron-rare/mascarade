"""MCP facade routes — FreeCAD, OpenSCAD, GitHub dispatch, knowledge-base, industrial."""
...
logger = logging.getLogger("mascarade.routes.mcp")
...
def _require_mcp_client(request: Request) -> McpRuntimeClient:
...
def _require_knowledge_base(request: Request) -> McpRuntimeClient:
    if not knowledge_base_auth_configured():
        raise HTTPException(
            status_code=503,
            detail=knowledge_base_status_detail(),
        )
    return _require_mcp_client(request)
...
async def knowledge_base_search(request: Request, q: str):
    if len(q) > 1000:
        raise HTTPException(status_code=400, detail="Search query too long (max 1000 chars)")
    client = _require_knowledge_base(request)
    try:
...
@router.get("/knowledge-base/pages/{page_id}")
async def knowledge_base_read_page(request: Request, page_id: str):
    client = _require_knowledge_base(request)
    try:
...
@router.post("/knowledge-base/pages/{page_id}/append")
async def knowledge_base_append(request: Request, page_id: str, req: KnowledgeBaseAppendRequest):
    client = _require_knowledge_base(request)
    try:
...
async def knowledge_base_create_page(request: Request, req: KnowledgeBaseCreateRequest):
    client = _require_knowledge_base(request)
    try:
...
@router.post("/agents/knowledge-scribe/run-and-push")
async def run_knowledge_scribe_and_push(request: Request, req: KnowledgeScribeRequest):
    try:
        agent = request.app.state.registry.get("knowledge-scribe")
    except KeyError:
        raise HTTPException(
...
@router.get("/mcp/github-dispatch/workflows")
async def github_dispatch_workflows(request: Request):
    client = _require_mcp_client(request)
    try:
...
@router.post("/mcp/github-dispatch/dispatch")
async def github_dispatch_dispatch(request: Request, req: GitHubDispatchRequest):
    client = _require_mcp_client(request)
    try:
...
@router.post("/mcp/github-dispatch/status")
async def github_dispatch_status(request: Request, req: GitHubDispatchStatusRequest):
    client = _require_mcp_client(request)
    try:
...
@router.get("/mcp/freecad/runtime")
async def freecad_runtime_info(
    request: Request, run_id: str | None = Query(default=None, max_length=64)
):
    client = _require_mcp_client(request)
    trace_run_id = run_id or new_run_id()
    try:
        payload = await client.freecad_get_runtime_info(
            run_id=trace_run_id, mode="freecad", step=0, agent_name="freecad",
        )
    except (McpCallError, McpServerUnavailable) as exc:
        raise _mcp_http_exception(exc) from exc
...
async def freecad_create_document(request: Request, req: FreeCADCreateDocumentRequest):
    client = _require_mcp_client(request)
    trace_run_id = req.run_id or new_run_id()
    try:
        payload = await client.freecad_create_document(
            req.output_path,
            name=req.name, primitive=req.primitive,
            length=req.length, width=req.width, height=req.height,
            run_id=trace_run_id, mode="freecad", step=0, agent_name="freecad",
        )
    except (McpCallError, McpServerUnavailable) as exc:
        raise _mcp_http_exception(exc) from exc
...
@router.post("/mcp/freecad/export")
async def freecad_export_document(request: Request, req: FreeCADExportDocumentRequest):
    client = _require_mcp_client(request)
    trace_run_id = req.run_id or new_run_id()
    try:
        payload = await client.freecad_export_document(
            req.document_path, req.output_path,
            run_id=trace_run_id, mode="freecad", step=0, agent_name="freecad",
        )
    except (McpCallError, McpServerUnavailable) as exc:
        raise _mcp_http_exception(exc) from exc
...
@router.post("/mcp/freecad/script")
async def freecad_run_script(request: Request, req: FreeCADRunScriptRequest):
    client = _require_mcp_client(request)
    trace_run_id = req.run_id or new_run_id()
    try:
        payload = await client.freecad_run_python_script(
            req.script, output_path=req.output_path,
            run_id=trace_run_id, mode="freecad", step=0, agent_name="freecad",
        )
    except (McpCallError, McpServerUnavailable) as exc:
        raise _mcp_http_exception(exc) from exc
...
@router.get("/mcp/openscad/runtime")
async def openscad_runtime_info(
    ...
    try:
        payload = await client.openscad_get_runtime_info(
            run_id=trace_run_id, mode="openscad", step=0, agent_name="openscad",
        )
    except (McpCallError, McpServerUnavailable) as exc:
        raise _mcp_http_exception(exc) from exc
    return {**payload, "run_id": trace_run_id}
...
@router.post("/mcp/openscad/validate")
async def openscad_validate_model(request: Request, req: OpenSCADValidateRequest):
    client = _require_mcp_client(request)
    trace_run_id = req.run_id or new_run_id()
    try:
        payload = await client.openscad_validate_model(
            req.source, run_id=trace_run_id, mode="openscad", step=0, agent_name="openscad",
        )
    except (McpCallError, McpServerUnavailable) as exc:
        raise _mcp_http_exception(exc) from exc
    return {**payload, "run_id": trace_run_id}
...
async def openscad_render_model(request: Request, req: OpenSCADRenderRequest):
    client = _require_mcp_client(request)
    trace_run_id = req.run_id or new_run_id()
    try:
        payload = await client.openscad_render_model(
            req.source, req.output_path,
            run_id=trace_run_id, mode="openscad", step=0, agent_name="openscad",
        )
    except (McpCallError, McpServerUnavailable) as exc:
        raise _mcp_http_exception(exc) from exc
...
@router.post("/mcp/openscad/export")
async def openscad_export_model(request: Request, req: OpenSCADRenderRequest):
    client = _require_mcp_client(request)
    trace_run_id = req.run_id or new_run_id()
    try:
...
@router.get("/mcp/industrial/servers")
async def industrial_mcp_servers(request: Request):
    client = _require_mcp_client(request)
    return {
        "items": [
            item
...
@router.get("/mcp/industrial/{server_key}/runtime")
async def industrial_mcp_runtime(request: Request, server_key: str):
...
@router.get("/mcp/industrial/{server_key}/resource")
async def industrial_mcp_resource(
    request: Request, server_key: str, uri: str = Query(..., min_length=1)
):
    if server_key not in INDUSTRIAL_MCP_SERVER_KEYS:
        raise HTTPException(
            status_code=404, detail=f"Unknown industrial MCP server '{server_key}'"
...
@router.get("/mcp/industrial/platform")
async def industrial_mcp_platform(request: Request):
    client = _require_mcp_client(request)
    inventory = [
        item
        for item in client.list_servers()
...
@router.post("/mcp/industrial/{server_key}/tools/{tool_name}")
async def industrial_mcp_tool(
    request: Request, server_key: str, tool_name: str, req: IndustrialMcpToolRequest
):
    if server_key not in INDUSTRIAL_MCP_SERVER_KEYS:
        raise HTTPException(
            status_code=404, detail=f"Unknown industrial MCP server '{server_key}'"
