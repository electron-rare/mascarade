...
class PagedAttentionManager:
    ...
    def _evict_to_cpu(self, num_blocks_needed: int) -> None:
        ...
        for block_id in blocks_to_evict:
            ...
            for (logical_id, idx), (
                location,
                phys_id,
            ) in self.logical_to_physical.items():
    ...
    def free_sequence(self, logical_id: int) -> None:
        ...
        blocks_to_free = [
            (location, block_id)
            for (lid, _), (location, block_id) in self.logical_to_physical.items()
            if lid == logical_id
        ]
        ...
        self.logical_to_physical = {
            k: v for k, v in self.logical_to_physical.items() if k[0] != logical_id
        }
        ...
        for logical_id in logical_ids:
            ...
            blocks = [
                ...
                for (lid, _), (
                    location,
                    block_id,
                ) in self.manager.logical_to_physical.items()
                    ...
                    if cpu_block is not None:
                        ...
                        for (lid, idx), (
                            ...
                            if loc == "cpu" and bid == block_id:
                                self.manager.logical_to_physical[(lid, idx)] = (
                                    "gpu",
                                    gpu_block_id,
                                )
