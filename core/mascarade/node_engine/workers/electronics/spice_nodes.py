...
class NetlistGeneratorNode(BaseNode):
    ...
    def __init__(self, config: NodeConfig | None = None, *, router: Any | None = None) -> None:
        """Initialize the netlist generator node.

        Args:
            config: Node-specific configuration parameters
            router: Optional Router instance for LLM-powered netlist generation
                via SpiceAgent. When None, falls back to template-based generation.
        """
    ...
    async def execute(self, inputs: dict[str, Any]) -> dict[str, Any]:
        ...
        # Attempt AI-powered netlist generation via SpiceAgent
        ai_netlist = await self._generate_with_spice_agent(
            circuit_description, device_models, target_simulator
        )

        if ai_netlist:
            logger.info("Using SpiceAgent-generated netlist")
            netlist_content = ai_netlist
        else:
            # Fallback: template-based netlist generation
            logger.info("SpiceAgent unavailable, using template-based generation")
            netlist_lines = [
            ...
            # Placeholder circuit (no AI available)
            netlist_lines.extend(
                [
                    "* Circuit netlist",
                    f"* Description: {circuit_description}",
                    "",
                ]
            )
    ...
    async def _generate_with_spice_agent(
        self,
        circuit_description: str,
        device_models: str | None,
        target_simulator: str,
    ) -> str | None:
        """Attempt to generate a netlist using SpiceAgent + LLM router.

        Returns the AI-generated netlist string, or None if the agent or
        ...
        try:
            ...
            # Build an enriched prompt that includes device models and simulator target
            enriched_description = circuit_description
            if device_models:
                enriched_description += f"\n\nUse the following device models:\n{device_models}"
            if target_simulator != "ngspice":
                enriched_description += f"\n\nTarget simulator: {target_simulator}"

            netlist = await agent.generate_netlist(enriched_description, router=self._router)
            if netlist and netlist.strip():
                return netlist
        except Exception:
            logger.warning(
                "SpiceAgent netlist generation failed, falling back to template",
                exc_info=True,
            )
...
class SimulateNode(BaseNode):
    ...
    async def execute(self, inputs: dict[str, Any]) -> dict[str, Any]:
        ...
        if netlist_obj and isinstance(netlist_obj, dict):
            ...
            raise ValueError("Either 'netlist' object or 'netlist_content' string must be provided")
        ...
        try:
            ...
            logger.info(f"Simulation completed: exit_code={exit_code}, success={success}")
...
class AnalyzeNode(BaseNode):
    ...
    def _extract_characteristics(self, netlist: str) -> dict[str, Any]:
        ...
        characteristics["unique_component_types"] = len(set(characteristics["components"]))

        return characteristics

    def _suggest_measurements(self, netlist: str, characteristics: dict[str, Any]) -> list[str]:
    ...
    def _suggest_improvements(self, netlist: str, characteristics: dict[str, Any]) -> list[str]:
        ...
        if "mosfet" in components or "transistor" in components:
            improvements.append("Ensure proper device models are included for semiconductors")
...
class DebugConvergenceNode(BaseNode):
    ...
    async def execute(self, inputs: dict[str, Any]) -> dict[str, Any]:
        ...
        diagnosis = self._diagnose_convergence_issue(error_message, netlist_text, simulation_output)
