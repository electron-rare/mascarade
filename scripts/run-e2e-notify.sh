#!/usr/bin/env bash
set -uo pipefail
HC_PING="https://hc.saillant.cc/ping/529ac446-df62-445d-a2d8-04e51871378f"
curl -fsS --retry 3 "$HC_PING/start" -o /dev/null 2>/dev/null
cd /home/clems/mascarade/web || { curl -fsS --retry 3 "$HC_PING/fail" -o /dev/null 2>/dev/null; exit 1; }
