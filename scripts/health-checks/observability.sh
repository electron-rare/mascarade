#!/usr/bin/env bash
# scripts/health-checks/observability.sh - Health check for observability profile services
set -euo pipefail
...
# ── Main ──
main() {
    info "Checking observability profile services..."
    echo ""
    ...
    if [[ $failed -eq 0 ]]; then
        ok "All observability services healthy (${#OBSERVABILITY_SERVICES[@]}/${#OBSERVABILITY_SERVICES[@]})"
        return 0
    else
        err "$failed/${#OBSERVABILITY_SERVICES[@]} observability services failed health check"
        return 1
    fi
}
