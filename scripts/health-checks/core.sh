#!/usr/bin/env bash
# scripts/health-checks/core.sh - Health check for core profile services
set -euo pipefail
...
# ── Main ──
main() {
    info "Checking core profile services..."
    echo ""
    ...
    if [[ $failed -eq 0 ]]; then
        ok "All core services healthy (${#CORE_SERVICES[@]}/${#CORE_SERVICES[@]})"
        return 0
    else
        err "$failed/${#CORE_SERVICES[@]} core services failed health check"
        return 1
    fi
}
