...
def done(name):
    if not os.path.isdir(RUNS):
        return False
    return any(
        d.startswith(name) and os.path.isfile(f"{RUNS}/{d}/manifest.json")
...
def train(name, path, domain):
    ...
    def fmt(ex):
        ...
        for c in ex.get("conversations", []):
            ...
            if not vl:
                continue
            if rl in ("system", "human", "user"):
                txt += f"<s>[INST] {vl} [/INST]"
            elif rl in ("assistant", "gpt"):
                txt += f" {vl}</s>"
        return {"text": txt}
    ...
    tr = SFTTrainer(
        ...
        args=SFTConfig(
            ...
            learning_rate=LR,
            warmup_steps=min(50, len(ds) // (BATCH * GA) // 10),
            logging_steps=25,
            save_steps=500,
            max_seq_length=SEQ,
    ...
    d = time.time() - s
    print(f"  DONE {d:.0f}s")
    m.save_pretrained(f"{rd}/lora")
    t.save_pretrained(f"{rd}/lora")
    mf = {
        "name": name,
        "base": BASE_MODEL,
        "dataset": path,
...
for r in results:
    print(f"  {r['name']}: {r['examples']} ex, {r['duration']:.0f}s")
json.dump(
    {"trained": len(results), "results": results},
    open(f"{RUNS}/queue_summary.json", "w"),
    indent=2,
)
