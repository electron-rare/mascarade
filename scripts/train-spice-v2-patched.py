...
print(
    f"[patch] LoraConfig patched (accepts {len(_valid_params)} params, strips unknown)"
)
...
ds = load_dataset(
    "json", data_files="finetune/datasets/spice_chat.jsonl", split="train"
)
...
json.dump(
    {"name": "mascarade-spice-v2", "examples": len(ds), "duration": dur, "output": RUN},
    open(f"{RUN}/manifest.json", "w"),
    indent=2,
)
print(f"Saved: {RUN}")
