...
module_agent_factory_cockpit_config() {
  ...
  AGENT_FACTORY_DCS_SANDBOX_PORT=$(input_value "Port sandbox DCS local" "${AGENT_FACTORY_DCS_SANDBOX_PORT:-4184}")
}
...
module_agent_factory_cockpit_compose() {
  ...
  echo "      context: ../agent-factory-cockpit"
  echo "      dockerfile: Dockerfile"
  echo "    container_name: mascarade-agent-factory-dcs-sandbox"
  echo "    restart: unless-stopped"
  echo "    env_file:"
  echo "      - .env"
  echo "    environment:"
  ...
  echo "      - python3"
  echo "      - -m"
  echo "      - agent_factory_cockpit.dcs_sandbox"
  echo "      - --host"
  echo "      - 0.0.0.0"
  echo "      - --port"
  echo "      - \"\${AGENT_FACTORY_DCS_SANDBOX_PORT:-4184}\""
  echo "      - --api-key"
  echo "      - \"\${AGENT_FACTORY_DCS_API_KEY:-demo-dcs-key}\""
  echo "      - --auth-header"
  echo "      - \"\${AGENT_FACTORY_DCS_AUTH_HEADER:-X-DCS-Key}\""
  echo "    expose:"
  echo "      - \"4184\""
  echo "    read_only: true"
  echo "    tmpfs:"
  ...
  echo "      test:"
  echo "        - CMD-SHELL"
  echo "        - python3 -c \"import urllib.request; urllib.request.urlopen('http://127.0.0.1:4184/health')\""
  echo "      interval: 15s"
  echo "      timeout: 5s"
  echo "      retries: 10"
