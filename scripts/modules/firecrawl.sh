...
module_firecrawl_compose() {
  ...
  firecrawl:
    ...
    healthcheck:
      test:
        - CMD-SHELL
        - IP=$$(hostname -i | awk '{print $$1}') && node -e 'if (!process.env.FIRECRAWL_API_KEY && !process.env.FIRECRAWL_API_URL) process.exit(1); const net = require("net"); const host = process.argv[1]; const socket = net.connect(3000, host); socket.on("connect", () => { socket.end(); process.exit(0); }); socket.on("error", () => process.exit(1)); setTimeout(() => process.exit(1), 3000);' "$$IP"
      interval: 15s
      timeout: 5s
      retries: 10
      start_period: 20s
