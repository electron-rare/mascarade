...
class ControlSystem:
    ...
    async def enable_agent(self, agent_name: str) -> Dict:
        ...
        return await self.api.request(
            "PUT", f"/v1/api/agents/{agent_name}", json={"enabled": True}
        )

    async def disable_agent(self, agent_name: str) -> Dict:
        """Disable an agent."""
        return await self.api.request(
            "PUT", f"/v1/api/agents/{agent_name}", json={"enabled": False}
        )
    ...
    async def set_provider_key(self, provider: str, api_key: str) -> Dict:
        ...
        return await self.api.request(
            "PUT", f"/v1/api/providers/{provider}/key", json={"api_key": api_key}
        )
    ...
    async def agent_management_menu(self) -> None:
        ...
        while True:
            ...
            for i, agent in enumerate(agents.get("agents", []), 1):
                ...
                table.add_row(
                    str(i), agent["name"], status, agent.get("type", "standard")
                )
            ...
            if choice == "b":
                ...
                try:
                    ...
                    if "error" not in result:
                        self.console.print(
                            f"[green]✓ Agent {agent_name} enabled[/green]"
                        )
                    else:
                ...
                try:
                    ...
                    if "error" not in result:
                        self.console.print(
                            f"[yellow]⚠️ Agent {agent_name} disabled[/yellow]"
                        )
                    else:
                        self.console.print(f"[red]✗ Error: {result['error']}[/red]")
                except Exception as e:
        ...
        while True:
            ...
            if choice == "b":
                ...
                if Confirm.ask("⚠️ Are you sure you want to restart P2P mesh?"):
                    ...
                    if "error" not in result:
                        self.console.print(
                            "[green]✓ P2P mesh restart initiated[/green]"
                        )
                    else:
                        self.console.print(f"[red]✗ Error: {result['error']}[/red]")
                    await asyncio.sleep(2)
