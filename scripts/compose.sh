...
write_env_file() {
...

    {
        ...
        if svc_selected "agent-factory-cockpit"; then
            ...
            echo "AGENT_FACTORY_DCS_SANDBOX_PORT=\"${AGENT_FACTORY_DCS_SANDBOX_PORT:-4184}\""
            echo "AGENT_FACTORY_DCS_PACK=\"${AGENT_FACTORY_DCS_PACK:-generic-rest}\""
            echo "AGENT_FACTORY_DCS_BASE_URL=\"${AGENT_FACTORY_DCS_BASE_URL:-http://agent-factory-dcs-sandbox:4184/api}\""
            echo "AGENT_FACTORY_DCS_AUTH_MODE=\"${AGENT_FACTORY_DCS_AUTH_MODE:-api-key}\""
            echo "AGENT_FACTORY_DCS_API_KEY=\"${AGENT_FACTORY_DCS_API_KEY:-demo-dcs-key}\""
            echo "AGENT_FACTORY_DCS_AUTH_HEADER=\"${AGENT_FACTORY_DCS_AUTH_HEADER:-X-DCS-Key}\""
            echo "AGENT_FACTORY_DCS_AUTH_SCHEME=\"${AGENT_FACTORY_DCS_AUTH_SCHEME:-}\""
            echo "AGENT_FACTORY_DCS_RESOURCE_READ_SNAPSHOT=\"${AGENT_FACTORY_DCS_RESOURCE_READ_SNAPSHOT:-read-snapshot}\""
            echo "AGENT_FACTORY_DCS_METHOD_READ_SNAPSHOT=\"${AGENT_FACTORY_DCS_METHOD_READ_SNAPSHOT:-GET}\""
            echo "AGENT_FACTORY_DCS_RESOURCE_WRITE_REQUEST=\"${AGENT_FACTORY_DCS_RESOURCE_WRITE_REQUEST:-write-request}\""
            echo ""
        fi
        ...
        if svc_selected "edge-proxy"; then
            ...
            echo "EDGE_PROXY_SEARCH_SERVER_NAME=\"${EDGE_PROXY_SEARCH_SERVER_NAME:-search.saillant.cc}\""
            echo "EDGE_PROXY_PAPERLESS_SERVER_NAME=\"${EDGE_PROXY_PAPERLESS_SERVER_NAME:-paperless.saillant.cc}\""
            echo "EDGE_PROXY_KARAKEEP_SERVER_NAME=\"${EDGE_PROXY_KARAKEEP_SERVER_NAME:-karakeep.saillant.cc}\""
